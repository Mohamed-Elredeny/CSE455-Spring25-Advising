...
import {getAllPlans, approvePlan, rejectPlan} from '../../../core/_requests'
...
const PlanDetails: FC = () => {
 ...
 const loadPlanDetails = useCallback( async () => {
  ...
  try {
    ...
    const foundPlan = response.data.find((p) => p.id !== undefined && p.id.toString() === planId)
    if (!foundPlan) {
...
const handleApprove = async () => {
  ...
  try {
    ...
    alert('Plan approved!')
    // Optionally reload plan details here
  } catch (err) {
